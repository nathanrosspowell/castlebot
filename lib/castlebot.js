...
CastleBot.prototype._onMessage = function (message) {
    ...
    if (this._isForCastleBot(message)){
        var message = "Donate: url: 'https://www.gofundme.com/39t6wr3c";
        if (this._isCastle37(message){
            message = "#Castle37 :eurpean_caste::37:
        }
...
CastleBot.prototype._passesCastleFilter = function (message) {
    ...
    return self._messageContains(message, 'castle bot') ||
           ...
           self._messageContains(message, this.name) ||
           self._messageContains(message, this.user.id);
};
...
CastleBot.prototype._isForCastleBot = function (message) {
    if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromCastleBot(message) && this._passesCastleFilter(message)) {
        return true;
    }
    return false;
};
