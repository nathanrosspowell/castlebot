...
CastleBot.prototype._onMessage = function (message) {
    ...
    if (self._isForCastleBot(message)){
        ...
        else if (self._isCastleUrl(message)){
            output = "Donate to " + self.goFundMe
...
CastleBot.prototype._replyWithDbList = function (originalMessage) {
    ...
    self._getDonors(false, function(info, donors){
        ...
        for (var i = donors.length - 1; i > donors.length - 6; --i) {
            var d = donors[i]
            message += "\n" + (i + 1) + ". " + d.name + " - " + info.currency + d.amount
            var msg = d.message;
            if (msg){
                message += "\n>" + msg
            }
...
CastleBot.prototype._messageContains = function (message, text) {
    ...
    var ret = lower.indexOf(text.toLowerCase()) > -1;
...
CastleBot.prototype._passesCastleFilter = function (message) {
    var self = this;
    var passes = self._isCastle37(message) ||
...
CastleBot.prototype._isUpdateRequest = function (message) {
    var self = this;
    return self._messageContains(message, ' update') || self._messageContains(message, 'castle total')
};

CastleBot.prototype._isDonorList = function (message) {
    var self = this;
    return self._messageContains(message, ' donor') || 
           self._messageContains(message, ' donors') ||
           self._messageContains(message, ' donations') 
};

CastleBot.prototype._isCastleUrl = function (message) {
    var self = this;
    return self._messageContains(message, ' url') || 
           self._messageContains(message, ' gofundme') ||
           self._messageContains(message, ' page') 
};
